...
app.get("/cargar-datos", async (req, res) => {
  try {
    ...
    const colecciones = [
      ...
      {
        modelo: CuentaCorriente,
        datos: cuentasCorrientes,
        nombre: "cuentasCorrientes",
      },
      { modelo: Lote, datos: lotes, nombre: "lotes" },
      {
        modelo: MovimientoStock,
        datos: movimientosStock,
        nombre: "movimientosStock",
      },
...
app.get("/", protegerRuta, async (req, res) => {
  try {
    ...
    const sinStock = productos.filter(
      (producto) => (producto.stock_actual || 0) === 0,
    ).length;
    const clientesDeudores = clientes.filter(
      (cliente) => (cliente.saldo_cuenta_corriente || 0) > 0,
    ).length;
    ...
    const ventasPorSemana = Object.values(ventasPorSemanaMap).sort((a, b) =>
      a.semana.localeCompare(b.semana),
    );
    const stockData = [
      {
        label: "Con stock",
        value: Math.max(productos.length - stockBajo - sinStock, 0),
      },
      ...
      metricas: [
        {
          ...
          valor: `$${ventasMesTotal.toLocaleString("es-AR")}`,
        },
