...
const ventasController = {
  ...
  async formularioVenta(req, res, next) {
    try {
      ...
      res.render('nuevaVenta', { titulo: 'Registrar Nueva Venta', clientes, productos, error: null });
    } catch (err) { next(err); }
  },
  ...
  async crear(req, res, next) {
    try {
      ...
      if (errors.length > 0) throw makeError(`Datos inválidos: ${errors.join(', ')}`, 422);
      ...
      for (const item of req.body.items) {
        ...
        const cantidadEnUnidades = item.tipo_venta === 'bulto'
        ...
        if (stockTotal < cantidadEnUnidades) {
          throw makeError(`Stock insuficiente para ${producto.nombre}. Disponible: ${stockTotal} unidades.`, 400);
        }
        ...
        itemsConLote.push({
          ...
          precio_facturado:  item.precio_unitario,
  ...
  async despachar(req, res, next) {
    try {
      const venta = await Venta.findById(Number(req.params.id));
      if (!venta) throw makeError('Venta no encontrada', 404);
      if (venta.estado !== 'pendiente') throw makeError(`La venta ya está ${venta.estado}`, 400);
